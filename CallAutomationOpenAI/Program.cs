...
// Cognitive Services endpoint for speech
...
string callTransferFailurePrompt = "It looks like I can't connect you to an agent right now, but we will get the next available agent to call you back as soon as possible.";
...
// Azure OpenAI — using DefaultAzureCredential (disableLocalAuth=true in this tenant)
...
var deploymentName = builder.Configuration.GetValue<string>("AzureOpenAIDeploymentModelName");
    ...
    ILogger<Program> logger) =>
{
    foreach (var eventGridEvent in eventGridEvents)
    {
        ...
        {
            logger.LogInformation("Play completed event received for connection id: {id}.", playCompletedEvent.CallConnectionId);
            if (!string.IsNullOrWhiteSpace(playCompletedEvent.OperationContext) &&
                (playCompletedEvent.OperationContext.Equals(transferFailedContext, StringComparison.OrdinalIgnoreCase)
            ...
            else if (!string.IsNullOrWhiteSpace(playCompletedEvent.OperationContext) &&
                     playCompletedEvent.OperationContext.Equals(connectAgentContext, StringComparison.OrdinalIgnoreCase))
            {
                ...
                else
                {
                    ...
                    TransferCallToParticipantResult result =
                        await answerCallResult.CallConnection.TransferCallToParticipantAsync(transferDestination);
                    logger.LogInformation("Transfer call initiated: {ctx}", result.OperationContext);
                }
            }
        });
        ...
        {
            logger.LogInformation("Play failed event received for connection id: {id}. Hanging up call...",
        ...
        {
            logger.LogInformation("Call transfer accepted event received for connection id: {id}.",
                callTransferAcceptedEvent.CallConnectionId);
        });

        client.GetEventProcessor().AttachOngoingEventProcessor<CallTransferFailed>(
            answerCallResult.CallConnection.CallConnectionId, async (callTransferFailedEvent) =>
        {
            logger.LogInformation("Call transfer failed event received for connection id: {id}.",
                callTransferFailedEvent.CallConnectionId);
            ...
            logger.LogError("Encountered error during call transfer, message={msg}, code={code}, subCode={subCode}",
                resultInformation?.Message, resultInformation?.Code, resultInformation?.SubCode);
            ...
            answerCallResult.CallConnection.CallConnectionId, async (recognizeCompletedEvent) =>
        {
            ...
            if (!string.IsNullOrWhiteSpace(speech_result?.Speech))
            {
                ...
                else
                {
                    ...
                    logger.LogInformation("Chat GPT response: {response}", chatGPTResponse);
            ...
            answerCallResult.CallConnection.CallConnectionId, async (recognizeFailedEvent) =>
        {
            ...
            if (MediaEventReasonCode.RecognizeInitialSilenceTimedOut.Equals(
                recognizeFailedEvent.ResultInformation.SubCode.Value.ToString()) && maxTimeout > 0)
            {
...
async Task HandleChatResponse(string chatResponse, CallMedia callConnectionMedia, string callerId,
    ILogger logger, string context = "OpenAISample")
{
...
async Task<bool> HasIntentAsync(string userQuery, string intentDescription, ILogger logger)
{
    ...
    logger.LogInformation("OpenAI results: isMatch={isMatch}, customerQuery='{query}', intentDescription='{intent}'",
        isMatch, userQuery, intentDescription);
    return isMatch;
}
